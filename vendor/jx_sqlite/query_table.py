...
from jx_base.container import type2container, Container
...
<<<<<<< .mine
        ...
        normalized_query = QueryOp.wrap(query, self)
        ...
        if normalized_query.format == "container":
            ...
            if len(normalized_query.edges) == 0 and len(normalized_query.groupby) == 0:
                ...
                for s in index_to_columns.values():
                    data[s.push_list_name][s.push_column_child] = unwrap(s.pull(result.data[0]))
...
<<<<<<< .mine
        return SelectOp.normalize_one(select)
||||||| .r1729
